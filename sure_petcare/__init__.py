...
PROD_ID = mk_enum(
    "PROD_ID",
    {"ROUTER": 1, "PET_FLAP": 3, "CAT_FLAP": 6},  # Pet Door Connect  # Cat Door Connect
)

LOC = mk_enum("LOC", {"INSIDE": 1, "OUTSIDE": 2, "UNKNOWN": -1})
...
class SurePetFlapAPI(object):
    ...
    def update_required(self):
        ...
        return (
            self.cache["AuthToken"] is None
            or self.households is None
            or self.household.get("pets") is None
        )
    ...
    def update_authtoken(self, force=False):
        ...
        data = {
            "email_address": self._init_email or self.cache["email"],
    ...
    def update_households(self, force=False):
        ...
        headers = self._create_header()
        response_household = self._api_get(
            _URL_HOUSEHOLD, headers=headers, params=params
        )
    ...
    def update_device_ids(self, household_id=None, force=False):
        ...
        household = self.households[household_id]
        if (
            household["default_router"] is not None
            and household["default_flap"] is not None
            and not force
        ):
            return
    ...
    def update_router_status(self, household_id=None):
        ...
        for rid in household["routers"]:
            ...
            response = self._get_data(url)
            self.cache["router_status"].setdefault(household_id, {})[rid] = response[
                "data"
            ]
    ...
    def update_timelines(self, household_id=None):
        ...
        if self.__read_only:
            raise SPAPIReadOnly()
        household_id = household_id or self.default_household
        params = (("type", "0,3,6,7,12,13,14,17,19,20"),)
        ...
        self.cache["pet_timeline"][household_id] = {
            pet_id: [
                x
                for x in self.cache["house_timeline"][household_id]
                if x["type"] == EVT.MOVE
                and tag_lut[x["movements"][0]["tag_id"]] == pet_id
            ]
    ...
    def _log_req(self, r, *args, **kwargs):
        ...
        self.req_rx_bytes += l
        print(
            "requests: %s %s -> %s (%0.3f kiB, total %0.3f kiB in %s requests)"
            % (
                r.request.method,
                r.request.url,
                r.status_code,
                l / 1024.0,
                self.req_rx_bytes / 1024.0,
                self.req_count,
            )
        )
...
class SurePetFlapMixin(object):
    ...
    def print_timeline(
        self, pet_id=None, name=None, entry_type=None, household_id=None
    ):
        """
                ...
                if entry_type is not None:
                    if movement["movements"][0]["tag_id"] == tag_id:
                        if movement["movements"][0]["direction"] == entry_type:
                            print(
                                movement["movements"][0]["created_at"],
                                pet_name,
                                DIRECTION[movement["movements"][0]["direction"]],
                            )
                else:
                    if movement["movements"][0]["tag_id"] == tag_id:
                        print(
